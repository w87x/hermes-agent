...
The `web_search` and `web_extract` tools support ten backend providers, configured via `config.yaml` or `hermes tools`:
...
```yaml
web:
  backend: firecrawl    # firecrawl | searxng | brave-free | ddgs | tavily | exa | parallel | xai | yandex | anysearch
```
